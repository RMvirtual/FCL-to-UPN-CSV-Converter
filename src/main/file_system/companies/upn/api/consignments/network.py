...
from src.main.file_system.companies.upn.api.keys.keys_with_constraints \
    import KeysWithConstraints


class NetworkConsignmentFiles(KeysWithConstraints):
    def __init__(self):
        super().__init__(self._keys_path(), self._constraints_path())

    def _keys_path(self) -> str:
        return self._absolute_path("consignments/network/keys.json")

    def _constraints_path(self):
        return self._absolute_path("consignments/network/constraints.json")
